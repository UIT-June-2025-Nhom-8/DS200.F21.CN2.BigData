...
> Notebook `04-robustness-test-kaggle.ipynb` sử dụng mapping `{"Real": 1, "Fake": 0}` — ngược với mapping huấn luyện `{"Real": 0, "Fake": 1}`. Do đó, JSON gốc từ Kaggle báo cáo Accuracy ≈ 1 − Acc_thực và AUC ≈ 1 − AUC_thực (ví dụ: baseline raw = 0.027, thực = 0.9733). File `tests/robustness/robustness_metrics.json` đã được hiệu chỉnh với công thức `acc_corrected = 1 − acc_raw`, `auc_corrected = 1 − auc_raw`. Cần sửa mapping trong notebook trước khi tái sử dụng.
