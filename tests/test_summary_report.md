...
Toàn bộ tập `test.csv` (47.480 mẫu) được đánh giá **8 lần**, mỗi lần áp dụng một biến đổi (perturbation) khác nhau lên ảnh gốc *trước khi* đưa vào mô hình. Perturbation được thực hiện ở cấp độ ảnh PIL, sau đó ảnh được đưa qua pipeline đã dùng trong notebook sinh metric: `LongestMaxSize(max_size=224)` → padding về `224×224` → `Normalize` → `ToTensor`. Vì vậy, các kết quả robustness trong báo cáo này phản ánh đúng preprocessing thực tế của notebook, thay vì phép co ảnh trực tiếp `Resize 224×224`.
