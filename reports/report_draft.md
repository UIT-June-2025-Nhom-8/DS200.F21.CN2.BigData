...
- Nhãn 0 (thật): thư mục có tên `real`, `Real`, hoặc `training_real`
- Nhãn 1 (giả): thư mục có tên `fake`, `Fake`, hoặc `training_fake`
...
**Schema CSV:** Mỗi hàng gồm hai trường — `image_path` (đường dẫn tuyệt đối tới file ảnh, kiểu chuỗi ký tự) và `label` (nhãn số nguyên: 0 = giả, 1 = thật).
