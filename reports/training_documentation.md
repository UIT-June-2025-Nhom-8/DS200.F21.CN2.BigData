...
- **Real (1):** ảnh chụp thật từ người thật
- **Fake (0):** ảnh tạo bởi AI (GAN, StyleGAN, Deepfake)
...
**Nhận xét:** Tập dữ liệu có class imbalance nhẹ (~47% Real vs ~53% Fake). Chênh lệch này đủ nhỏ để không cần điều chỉnh trọng số loss (`pos_weight`), nhưng vẫn cần theo dõi recall của lớp Real trong quá trình đánh giá.
...
| CoarseDropout             | max_holes=8, kích thước 16×16px     | 0.3      | Ngăn học thuộc artifact cục bộ |
...
| Thành phần          | Lý do                                                    |
|---------------------|----------------------------------------------------------|
| 1280 → 256          | Đủ rộng để học decision boundary Real/Fake               |
...
|-------|--------------------|------------------------|---------------|----------|
| 1     | Freeze Backbone    | Head only              | 1e-4          | Học decision boundary cơ bản |
...
- Precision > Recall → mô hình thiên về thận trọng (ít false positive hơn false negative)
- Kết quả cải thiện rõ rệt qua từng stage: 86.6% → 94.5% → 97.3%

**Hạn chế cần lưu ý:**
- Tập dữ liệu có class imbalance nhẹ (53% Fake) → precision cao có thể một phần do mô hình thiên về dự đoán Fake
